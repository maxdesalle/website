...
Compared to these other browsers, **Firefox stands out as it's not Chromium-based**. But we want to get one step further by not installing Firefox itself, but instead, install a fork (a modified copy) of Firefox called [**LibreWolf**](https://librewolf-community.gitlab.io/).
...
If you are not convinced by ProtonMail, [PrivacyTools](https://privacytools.io) has a great overview.
...
There are [a lot of encrypted DNS resolvers out there](https://privacytools.io/providers/dns/), but the one I use and recommend is:
